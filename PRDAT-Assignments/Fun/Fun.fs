...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Call(Var f, eArgs) -> 
      ...
      | Closure (f, x, fBody, fDeclEnv) ->
        let variableValues = List.foldBack (fun arg acc -> Int(eval arg env) :: acc) eArgs []
        let variablesWithValues = List.zip x variableValues
        let fBodyEnv = variablesWithValues @ ((f, fClosure) :: fDeclEnv)
